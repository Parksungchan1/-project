...
3. **배포** — Railway에 배포 완료 (백엔드 + 프론트엔드 모두). 절차는 [`RAILWAY_DEPLOY.md`](./RAILWAY_DEPLOY.md) 참고. 단, 무료 트라이얼 기간(23일 또는 $5 크레딧) 한정이라 장기 운영하려면 유료 전환 또는 다른 호스팅으로 이전이 필요함. **새 장소에서 프린터만 새로 연결**하려면(백엔드는 이미 떠 있으므로 재배포 불필요) [`PRINTER_SETUP.md`](./PRINTER_SETUP.md) 참고.
